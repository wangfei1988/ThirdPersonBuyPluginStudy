...
{
    /// <summary>
    /// jump over something trigger
    /// </summary>
    public class JumpOverTrigger : Trigger
    {
        /// <summary>
        ...
        public override bool start(TPCharacter character, IKHelper limbsIK, bool use, bool secondaryUse, bool jump, float v, float h)
        {
            ...
            if (turnToLedge)
            {
                ...
                else m_TriggerInfo.avatarTarget = AvatarTarget.RightHand;


                m_TriggerInfo.transform = Target;
                m_TriggerInfo.pos_offset = closest - Target.position;
                m_TriggerInfo.stateName = "Base Layer.TRIGGERS.JumpOver";
                character.disableMove = true;
                character.rigidBody.velocity = Vector3.zero;
                m_TriggerInfo.doMatchTarget = true;
                character.OnLerpEnd = () =>
                {
                    character.animator.SetBool(/*"pJumpOver"*/HashIDs.JumpOverBool, true);
            ...
            else
            {
                ...
                float forwAmt = character.forwardAmount; // Mathf.Min(character.forwardAmount, 0.5f);
        ...
        public override void end(TPCharacter character, IKHelper limbsIK)
        {
            ...
            character.animator.SetBool(/*"pJumpOver"*/HashIDs.JumpOverBool, false);
        }
